...
fn get_op_fmt_string(op: &Op, arg: u16) -> String {
    let instr_string: String = format!("{:?}", op.info.instruction);
    ...
    pub fn read(&mut self, offset: usize) {
        ...
        while let Some((pc, &byte)) = it.next() {
            ...
            let arg: u16 = match arg_count {
                ...
                1 => {
                    let (_, &arg) = it.next()
                        .expect("Unexpected EOF when parsing");
                    arg as u16
                },

                2 => {
                    let (_, &arg_lo) = it.next()
                        .expect("Unexpected EOF when parsing");

                    let (_, &arg_hi) = it.next()
                        .expect("Unexpected EOF when parsing");
                    
                    ((arg_hi as u16) << 8) + (arg_lo as u16)
                }

                _ => { panic!("Parsing error: invalid arg_count"); }
            };
