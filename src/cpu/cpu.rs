...
impl Cpu {
    ...
    pub fn adc(&mut self) {
        ...
        let a = self.regs.a;
        let m = self.operand;
        let res = a as u16 + m as u16;
        self.regs.a = res as u8;
        println!("a: {:#04x}, m: {:#04x}, res: {:#04x}",
                 a, m, res as u8);

        if is_neg(a) == is_neg(m) {
                self.set_flag(Flag::V, is_neg(res as u8) != is_neg(a));
        }

        self.update_nz_flags();
        self.set_flag(Flag::C, (res & 0xFF00) > 0);
    }
